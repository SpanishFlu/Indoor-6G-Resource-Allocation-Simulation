function ds_info = build_waveform_dataset(cfg, trace, slot_features, slot_class_id)
...
%
%   slot_features/slot_class_id (optional): precomputed ground-truth
%   labels from dataset/classify_all_slots.m — the SAME labels PF's
%   Method A/B use for their live scheduling decisions. Passing these in
%   guarantees the exported CSV is consistent with what the scheduler
%   actually saw, instead of an independent re-classification with its
%   own random draws. If omitted, this function derives them itself
%   (legacy behavior).
%
%   Pipeline:
...
%   TR 38.901 Indoor Factory delay-spread model.
    ...
    % =====================================================================
    % STEP 1: Slot features/labels — reuse shared ground truth if supplied
    % =====================================================================
    if nargin >= 4 && ~isempty(slot_features) && ~isempty(slot_class_id)
        fprintf('Using shared ground-truth classification from classify_all_slots (%d slots)...\n', T);
        Features   = slot_features;
        ClassLabel = slot_class_id;
    else
        fprintf('Extracting features from %d simulation slots...\n', T);
        [Features, ClassLabel] = classify_all_slots(cfg, trace);
    end
