...
%   but without requiring a simulation trace (standalone generation).
    ...
    % ----- Service assignment from same user pool (eMBB/URLLC only) -----
    r_svc   = rand(Nu, 1);
    service = zeros(Nu, 1);  % 1=eMBB, 2=URLLC
    service(r_svc <  cfg.P_eMBB) = 1;
    service(r_svc >= cfg.P_eMBB) = 2;

    N_eMBB  = sum(service == 1);
    N_URLLC = sum(service == 2);
    N_mMTC  = 0;   % mMTC removed: eMBB/URLLC-only system
    ...
    % ----- Feature vector (N_mMTC kept as 0 for column compatibility) -----
    feat = [mu_tau, sigma2_tau, mu_D, sigma2_D, N_eMBB, N_URLLC, N_mMTC];

    % ----- Classification -----
    numerology = assign_numerology(N_eMBB, N_URLLC, mu_D);
