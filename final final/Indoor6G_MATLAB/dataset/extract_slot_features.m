...
%     F7: Number of mMTC users (always 0 — mMTC removed, kept as a
%         column for structural compatibility with the dataset export)
%
...
%     - User counts come from actual traffic, scaled to fill the full
%       Nu-user pool between eMBB and URLLC (no mMTC remainder)
    ...
    % =====================================================================
    % FEATURES 5, 6, 7 — Service user counts from actual traffic (eMBB/URLLC
    % only — mMTC removed, so the full Nu-user pool splits between the two)
    % =====================================================================
    ...
    % No mMTC remainder: URLLC absorbs whatever eMBB doesn't take, so the
    % pool always sums exactly to Nu.
    scale = Nu / max(cfg.n_robots, 1);
    N_eMBB  = min(round(n_embb_base * scale), Nu);
    N_URLLC = Nu - N_eMBB;
    N_mMTC  = 0;   % mMTC removed; kept as a column for compatibility
