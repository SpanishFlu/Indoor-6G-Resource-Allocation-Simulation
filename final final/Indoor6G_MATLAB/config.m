...
%   Returns a struct with all parameters matching the Python CFG dictionary.
    ...
    % ----- dedicated per-robot traffic role (no mixed eMBB/URLLC robots) -----
    %   Robot 1            -> eMBB only
    %   Robots 2, 3         -> URLLC only
    CFG.n_embb_robots  = 1;
    CFG.n_urllc_robots = 2;
    assert(CFG.n_embb_robots + CFG.n_urllc_robots == CFG.n_robots, ...
        'n_embb_robots + n_urllc_robots must equal n_robots');
    ...
    % (mMTC removed: this system is eMBB/URLLC-only, no IoT sensor tier)

    % ----- Indoor Factory service probabilities (mMTC removed) -----
    %   Mirrors the dedicated robot split above (1 eMBB : 2 URLLC).
    CFG.P_eMBB  = 1/3;
    CFG.P_URLLC = 2/3;
